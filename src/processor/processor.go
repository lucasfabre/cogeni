...
// 1. Reads the source file from disk (with caching).
// 2. Extracts all Lua code embedded within <cogeni> blocks.
// 3. Executes the extracted Lua code in a pooled LuaRuntime.
// 4. Captures outputs (via write() calls) and schedules them for commitment.
func ProcessFile(cfg *config.Config, filePath string, c *Coordinator) error {
	...
	if luaScript == "" {
		// If clean mode is on, we ensure all generated blocks are emptied even if no script exists.
		if c.CleanMode {
	...
	defer c.runtimePool.Release(rt) // Return to pool when done

	// Configure hooks to allow the Lua script to trigger recursive processing
	// and wait for files that are being generated by other concurrent tasks.
	...
	// Set global variables for the script context.
	...
	// Wait for any async tasks started by the script to complete.
	rt.Schedule()
...
// It handles redirection (outfile/outtag) and dependency recording between files.
func (c *Coordinator) CaptureResults(rt *luaruntime.LuaRuntime, sourcePath string) error {
	// fileChanges maps target absolute paths to a map of (block_id -> content).
	fileChanges := make(map[string]map[string]string)
	// fullFileOverwrites tracks if a file should be completely overwritten (no tag replacement).
	...
	for id, out := range rt.Output {
		...
		// Determine the destination based on outfile/outtag calls in Lua.
		...
		// Record that the source script depends on the output file (for dependency tracking).
	...
	// Batch apply changes to the coordinator's in-memory results buffer.
...
// It uses an in-memory buffer in the coordinator to allow multiple passes or recursive updates.
func (c *Coordinator) ApplyChangesToBuffer(path string, outputs map[string]string, isFullFile bool) error {
	var currentContent string

	// Check the coordinator buffer first, then fall back to disk.
	...
	if isFullFile {
		// Complete overwrite: ignore tags, just take the first output.
		if c.CleanMode {
			newContent = ""
		} else {
		...
		// Block replacement: replace content between <generated> tags.
...
// It automatically handles and strips leading comment characters (e.g., "#", "//", "--").
	...
	for scanner.Scan() {
		...
		if !inside {
			if strings.Contains(trimmedLine, "<cogeni>") && !strings.Contains(trimmedLine, "<generated") {
				inside = true
				// Capture the exact characters before <cogeni> to strip them from subsequent lines.
		...
		// Strip prefix to recover the raw Lua code.
		code := line
...
// If an ID from the outputs map matches, its value is injected.
// If cleanAll is true, all cogeni-generated blocks are emptied.
func ReplaceGeneratedBlocks(content string, outputs map[string]string, cleanAll bool) string {
	...
	for scanner.Scan() {
		...
		if !inside {
			...
			// Detect start of a generated block.
		...
		// Detect end of a generated block.
		if strings.Contains(trimmedLine, "</cogeni>") {
			inside = false
			result.WriteString(line)
			result.WriteString("\n")
